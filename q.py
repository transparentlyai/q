...
def read_config_file() -> Tuple[Optional[str], str, Dict[str, str]]:
    ...
    if os.path.exists(CONFIG_PATH):
        ...
        if os.path.exists(example_config_path):
            ...
            console.print(
                "Make sure your API key starts with 'sk-ant-api' for Claude API v1 format.",
                style="info",
            )
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Send a question to Claude and get the response"
    )
    ...
    parser.add_argument(
        ...
        "-w",
        action="store_true",
        help="Show context and ask for confirmation before sending to Claude",
    )
    parser.add_argument(
        "--no-empty",
        "-e",
...
def get_initial_question(
    args: argparse.Namespace, prompt_session: PromptSession, history: FileHistory
) -> str:
    """
...
def handle_claude_error(e: Exception) -> None:
    ...
    if isinstance(e, anthropic.APIStatusError):
        if e.status_code == 401:
            ...
            console.print(
                f"Error communicating with Claude (Status {e.status_code}): {e.message}",
                style="error",
            )
...
def confirm_context(prompt_session: PromptSession, system_prompt: str) -> bool:
    ...
    while True:
        response = (
            get_input("Send this context to Claude? [Y/n] ", session=prompt_session)
            .strip()
            .lower()
        )
