...
from q_cli.cli.context_setup import handle_context_confirmation, configure_file_tree, setup_context_and_prompts
...
class TestContextSetup:
    ...
    def test_handle_context_confirmation_empty_context(self, mock_confirm_context):
        ...
        assert not any(call == call("\n[bold cyan]Sanitized Context:[/bold cyan]") for call in console.print.call_args_list)
    ...
    def test_project_context_setup(
        ...
        with patch("q_cli.cli.context_setup.build_context", 
                  return_value=(context, context_manager)) as mock_build_context:
            with patch("builtins.open", m_open):
                # Call function
                result_context_manager, result_sanitized_context, result_system_prompt = setup_context_and_prompts(
                ...
                assert "Additional project information can be found in the following files:" in projectcontext
